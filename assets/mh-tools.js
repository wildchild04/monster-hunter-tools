...
// ../build/dev/javascript/mh_tools/mh_tools.mjs
var FILEPATH = "src/mh_tools.gleam";
var Model = class extends CustomType {
  constructor(input2, output) {
    super();
    this.input = input2;
    this.output = output;
  }
...
function update2(model, msg) {
  if (msg instanceof UpdateInput) {
    let val = msg[0];
    return new Model(val, model.output);
  } else {
    return new Model(model.input, uppercase(model.input));
  }
}
function view(model) {
  return div(
